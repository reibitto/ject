...
object WordSearchStrategy extends Enum[WordSearchStrategy] {
  ...
  /** Every inflected form of every conjugable entry is generated at index time
    ...
    * surface form matches directly via a term query against that field. Larger
    * index, no query-time ambiguity to resolve.
    */
  ...
  /** No inflected forms are indexed. A query is deinflected back to candidate
    * dictionary forms at search time, and each candidate is looked up directly
    * against the (much smaller) dictionary-form fields. Smaller index, but a
    * single query can fan out into several candidate lookups, and an incorrect
    * candidate can occasionally collide with an unrelated real word.
    */
